...
export default function Posts() {
  const {
    ...
    posts: s.posts,
    postsMeta: s.postsMeta,
    postsLoading: s.postsLoading,
    fetchPosts: s.fetchPosts,
    updatePostStatus: s.updatePostStatus,
  }));
  ...
  const handleStatusChange = async (postId, status) => {
    ...
    // fetchPosts will update state; if you want immediate refetch:
    fetchPosts(currentPage, POSTS_PER_PAGE);
  };
          ...
          {posts.map((post) => (
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 p-4 text-white z-10 w-full">
                ...
                <p className="text-xs mt-1">Posted by: {post.user?.fullName || post.user?.username}</p>
                <p className="text-xs">Date: {post.createdAt ? new Date(post.createdAt).toLocaleDateString() : ""}</p>
